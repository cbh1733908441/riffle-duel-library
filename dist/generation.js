...
function renderDocument(){if(!current)return;const html=documentText()?markdown(documentText()):'<div class="document-empty"><span>✦</span><h3>策划正文将在这里展开</h3><p>正在查阅资料时，你可以先在右侧提出要求。正文按章节更新。</p></div>';if($('#plan-preview').innerHTML!==html)$('#plan-preview').innerHTML=html;$('#plan-preview').hidden=editing;$('#plan-editor').hidden=!editing;$('#edit-plan').textContent=editing?'完成直接编辑':'直接编辑';$('#edit-plan').disabled=current.published||selectedView!=='current';document.querySelectorAll('[data-doc-view]').forEach(b=>b.classList.toggle('selected',b.dataset.docView===selectedView));}
...
function renderState(j,id){if(id!==epoch)return;const changedJob=current&&current.id!==j.id;current=j;const running=ACTIVE.includes(j.status);
 ...
 $('#chat-note').textContent=j.status==='planning'||j.status==='queued'?'可以现在提交要求，初稿完成后依次处理。':j.status==='revising'?'正在修改；可继续编辑正文或排队提交下一条要求。':'写下想改的规则，AI 会给出可预览、可应用的修改候选。';
 $('#chat-log').innerHTML=(j.messages||[]).map(m=>'<div class="chat-turn"><p class="chat-user">'+esc(m.text)+'</p><p class="chat-answer">'+esc(({queued:'已排队 · 等待当前草稿',running:'正在修改…',done:m.response,failed:m.response})[m.status]||m.status)+'</p></div>').join('')||'<p class="chat-empty">例如：把一局缩短到 90 秒；增加 NPC 的失误；去掉复杂养成。</p>';
 $('#proposal-bar').hidden=!j.proposalId;$('#proposal-note').textContent=j.proposalId&&j.proposalBaseRevision!==j.planRevision?'候选基于版本 '+j.proposalBaseRevision+'，正文已更新。请先对照；应用会替换正文并保留历史版本。':'AI 候选不会自动覆盖正文。应用后会保留历史版本。';$('#proposal-apply').disabled=running||j.published;$('#proposal-discard').disabled=running||j.published;
 ...
 $('#image-notice').textContent=j.proposalId?'还有修改候选待处理，请先应用或放弃。':j.status==='imaging'?'正在使用版本 '+j.imageRequestedRevision+' 生图；继续编辑会让本次图片对应旧版本。':j.imageStale?'策划已更新，这张图对应旧版本。可按最新策划重新生成。':j.imageReady?'参考图已完成，可下载并交给开发。':'检查策划与生图提示词后，再点击生成参考图。';
...
async function mount(g,m){reset();const id=epoch,modeId=m?.id||'',host=$('#generation-panel');if(!host)return;cacheKey='riffle.workspace.v2:'+g.id+'/'+modeId;
 ...
 <section data-work-pane="2" hidden><div class="work-toolbar"><div><p class="eyebrow">PLAN STUDIO</p><h3>把想法变成可制作的规则</h3></div><div class="toolbar-actions"><select id="job-history" aria-label="策划记录"><option value="">当前策划</option></select><button id="new-plan" class="secondary">新建策划</button><button id="continue-image" class="primary" disabled>下一步：参考图 →</button></div></div><div class="plan-studio"><section class="plan-paper"><div class="paper-tools"><div class="doc-views"><button data-doc-view="current" class="selected">当前正文</button><button data-doc-view="draft" id="doc-draft" hidden>AI 生成中</button><button data-doc-view="proposal" id="doc-proposal" hidden>修改候选</button></div><button id="edit-plan" class="secondary">直接编辑</button></div><div id="proposal-bar" class="proposal-bar" hidden><span id="proposal-note">AI 候选不会自动覆盖正文。应用后会保留历史版本。</span><button id="proposal-preview" class="text-button">预览候选</button><button id="proposal-apply" class="primary">应用候选到正文</button><button id="proposal-discard" class="text-button">放弃候选</button></div><div id="conflict-box" class="conflict-box" hidden><b>正文在别处更新了。你的编辑仍在下方。</b><details><summary>对照最新已保存正文</summary><pre id="conflict-latest"></pre></details><button id="save-over-latest" class="secondary">保留我的编辑为新版本</button></div><article id="plan-preview" class="plan-prose"><div class="document-empty"><span>✦</span><h3>开始你的第一份策划</h3><p>点击「生成策划案」，再在这里阅读与编辑。</p></div></article><label class="sr-only" for="plan-editor">策划正文</label><textarea id="plan-editor" maxlength="60000" spellcheck="false" hidden></textarea><div class="paper-footer"><span id="plan-save-state">尚未生成</span><button id="save-plan" class="text-button">保存正文</button><button id="download-plan" class="text-button" disabled>下载策划</button><button id="plan-retry" class="secondary" hidden>重试策划</button></div><div class="version-tools"><select id="plan-history" aria-label="历史版本"><option value="">版本记录</option></select><button id="restore-version" class="text-button" disabled>恢复所选版本</button></div></section><section class="revision-sidebar"><p class="eyebrow">和策划助手一起修改</p><h3>说说你想怎么改</h3><p id="chat-note">正文生成时也可以提出要求。</p><div id="chat-log" aria-live="polite"></div><label for="chat-message">修改要求</label><textarea id="chat-message" rows="5" maxlength="4000" placeholder="例如：只保留三种技能，增加一次失败后立即重来的机会。"></textarea><button id="send-revision" class="primary">发送修改要求 ↑</button><small>直接编辑自动保存；AI 修改先生成候选，应用后才改变正文。</small></section></div></section>
